...
// Map functions
  ...
  buildDirections(geolocMessages: Message[], directionsDisplayStore: any, routesColor: string, travelMode: string) {
    ...
    for (const i in directionsDisplayStore) {
      directionsDisplayStore[i].setMap(null);
    }
    directionsDisplayStore = [];
    console.log('--------------------------------');

    let messages: Message[] = [];
    const startCoord = geolocMessages[0].geoloc[0];
    const endCoord = geolocMessages[geolocMessages.length - 1].geoloc[0];

    const routes = Math.floor(geolocMessages.length / 23) !== 0 ? Math.floor(geolocMessages.length / 23) : 1;
    console.log('Number of routes (23 geolocs per routes)', routes);
    for (let i = 0; i < routes; i++) {
      let waypoints = [];
      messages = geolocMessages.slice(i * 23, (i + 1) * 23);
      ...
      if (waypoints.length <= 23) {
        ...
        this._googleMapsAPIWrapper.getNativeMap().then((map) => {
          ...
          directionsDisplayStore.push(directionsDisplay);
