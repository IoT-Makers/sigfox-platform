...
export class CustomDashboardComponent implements OnInit, OnDestroy {
  ...
  private SIGFOX_DEVICE_ID_LENGTH = 8;

  private device: Device;
  private devices: Array<Device> = [];
  private selectedDevicesIds: Array<String> = [];
  private setCategoriesDevices: Array<object> = [];

  private category: Category;
  private categories: Array<Category> = [];

  public selectCategoriesDevices: Array<object>;
  ...
  newWidgetIcon(icon: any): void {
    this.newWidget.icon = icon.id;
  }

  newWidgetType(widgetType: any): void {
  ...
  removedSelectedCategoriesDevices($event: any, type: string): void {
    if ($event.id.length > this.SIGFOX_DEVICE_ID_LENGTH) {
      this.userApi.getDevices(this.user.id, {where: {categoryId: $event.id}}).subscribe( (devices: Device[]) => {
        devices.forEach( (device: Device) => {
          this.selectedDevicesIds.splice(this.selectedDevicesIds.indexOf(device.id), 1);
        });
        this.setFilter(type);
      });
    } else {
      this.selectedDevicesIds.splice(this.selectedDevicesIds.indexOf($event.id), 1);
      this.setFilter(type);
    }
  }

  selectedCategoriesDevices($event: any, type: string): void {
    if ($event.id.length > this.SIGFOX_DEVICE_ID_LENGTH) {
      this.userApi.getDevices(this.user.id, {where: {categoryId: $event.id}}).subscribe( (devices: Device[]) => {
        devices.forEach( (device: Device) => {
          this.selectedDevicesIds.push(device.id);
        });
        this.setFilter(type);
      });
    } else {
      this.selectedDevicesIds.push($event.id);
      this.setFilter(type);
    }
  }
  ...
  setFilter(type: string): void {
    ...
    this.selectedDevicesIds.forEach( (deviceId: string) => {
      this.newWidget.filter.where.or.push({id: deviceId});
    });
    console.log('Added filter', this.newWidget.filter);
  }

  addWidget(): void {
    //this.newWidget.DashboardId = this.dashboard.id;
  ...
  editWidget(widget): void {
    ...
    this.setCategoriesDevices = this.newWidget.filter.where.or;
    this.setCategoriesDevices.forEach((item: any, index: number) => {
      this.setCategoriesDevices[index] = {id: item.id, text: item.id};
    });
    console.log(this.setCategoriesDevices);
  ...
  loadSelectFilters(): void {
    // Categories and devices list
    this.selectCategoriesDevices = [];
    ...
    this.userApi.getCategories(this.user.id).subscribe(categories => {
      ...
      this.categories.forEach((category: Category) => {
        ...
        this.selectCategoriesDevices.push(item);
      });
    });
    ...
    this.userApi.getDevices(this.user.id).subscribe(devices => {
      ...
      this.devices.forEach((device: Device) => {
        const item = {
          id: device.id,
          text: device.name ? device.name : device.id
        };
        this.selectCategoriesDevices.push(item);
      });
    });
