import {Component, ElementRef, OnDestroy, OnInit, Inject, QueryList, ViewChild, ViewChildren} from '@angular/core';
import {ActivatedRoute, Router} from '@angular/router';
import {DashboardApi, UserApi} from '../../shared/sdk/services/index';
import {Category, Dashboard, Device, Connector, User, Widget, Parser} from '../../shared/sdk/models/index';
import {ToasterConfig, ToasterService} from 'angular2-toaster';
import {Message, Organization} from '../../shared/sdk/models';
import * as _ from 'lodash';
import * as moment from 'moment';
import {MessageApi, DeviceApi, OrganizationApi, ParserApi} from '../../shared/sdk/services/custom';
...
export class CustomDashboardComponent implements OnInit, OnDestroy {
  ...
  private selectCategoryPropsSettings = {
    singleSelection: false,
    ...
    this.subscriptions.push(this.route.parent.parent.params.subscribe(parentParams => {
      if (parentParams.id) {
        this.userApi.findByIdOrganizations(this.user.id, parentParams.id).subscribe((organization: Organization) => {
          ...
          console.log("THIS ORGA 1", this.organization);

          this.setup();
        });
      } else {
        console.log("THIS ORGA 2", this.organization);
  ...
  editDevice(widget, deviceTest) {
    this.edit = true;
    this.deviceToEdit = deviceTest;
    this.actualWidget = widget;
  }
  ...
  setWidgetFilter(): void {
    ...
    // Table
      ...
      this.newWidget.filter = {
        limit: 100,
        order: 'messagedAt DESC',
        include: ['Parser', 'Category', 'Organizations' ,'Geolocs', {
          relation: 'Messages',
          scope: {
            limit: 1,
            order: 'createdAt DESC'
