  ...
  private triggerByDevice(data_parsed: Array<any>, device: any, req: any, next: Function): void {
    ...
    data_parsed.forEach((p: any) => {
      ...
      alerts.forEach((alert: any, index: any) => {
        ...
        if (alert.active && (alert.key === p.key)) {
          // Verify conditions for the alert to be triggered
          if (
            (alert.value_exact && p.value === alert.value_exact)
            || (alert.value_min && alert.value_max && p.value >= alert.value_min && p.value <= alert.value_max)
            || (alert.value_less && p.value < alert.value_less)
            || (alert.value_more && p.value > alert.value_more)
          ) {
            // Trigger alert
