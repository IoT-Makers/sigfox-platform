...
class Device {
  ...
  private addDeviceMessagesToOrganization(
    deviceId: string,
  ...
  public afterRemoteLinkOrganizations(
    ctx: any,
    data: any,
    next: Function
  ): void {
  ...
  private removeDeviceMessagesFromOrganization(
    deviceId: string,
    organizationId: string
  ): void {
    const Message = this.model.app.models.Message;
  ...
  public afterRemoteUnlinkOrganizations(
    ctx: any,
    data: any,
    next: Function
  ): void {
    // console.log('device afterRemoteUnlinkOrganizations');
    this.removeDeviceMessagesFromOrganization(
      data.deviceId,
      data.organizationId
    );
    next();
  ...
  public getMessagesFromSigfoxBackend(
    deviceId: string,
    limit: number,
    before: number,
    req: any,
          ...
          if (connector) {
            ...
            this.model.app.dataSources.sigfox
              ...
              .then((result: any) => {
                ...
                result.data.forEach(
                  (messageInstance: any, msgCounter: number) => {
                    reception = [];
                    ...
                    message.id =
                      messageInstance.device +
                      messageInstance.time +
                      messageInstance.seqNumber;
                    ...
                    Message.create(
                      message,
                      (err: any, messagePostProcess: any) => {
                        // callback
    ...
    Device.findOne(
      ...
      function(err: any, device: any) {
        if (err) console.error(err);
          ...
          attendedNbMessages =
            device.Messages[0].seqNumber -
            device.Messages[device.Messages.length - 1].seqNumber +
            1;
          if (
            device.Messages[device.Messages.length - 1].seqNumber >
            device.Messages[0].seqNumber
          ...
          device.successRate = (
            (device.Messages.length / attendedNbMessages) *
            100
          ).toFixed(2);
              ...
              if (category.Organizations) {
                ...
                category.Organizations.forEach((org: any) => {
                  oldDevice.Organizations.remove(org.id, () => {
                    Device.removeDeviceMessagesFromOrganization(
                      oldDevice.id,
                      org.id
                    );
                  });
