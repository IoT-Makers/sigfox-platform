...
class user {
  ...
  constructor(public model: any) {
    ...
    model.on('resetPasswordRequest', function(info: any) {
      ...
      const resetUrl =
          baseUrl + '/#/reset-password?access_token=' + info.accessToken.id;
      // Prepare a loopback template renderer
      const renderer = loopback.template(
          path.resolve(__dirname, '../../server/views/resetPassword.ejs'));
  ...
  public afterRemoteChangePassword(ctx: any, reuslt: any, next: Function):
      void {
    const userId = ctx.args.id;
    this.model.findById(userId, {}, (err: any, userInstance: any) => {
      if (err) {
        console.error(err);
        ...
        if (devAccessTokens) {
          this.model.app.models.AccessToken.create(
              devAccessTokens, (error: any, result: any) => {
                if (err)
                  next(error, result);
                else
                  next();
                console.log(
                    'Successfully restored devAccessTokens in AccessToken model.');
              });
        } else
          next();
      }
  ...
  public beforeSave(ctx: any, next: Function): void {
    if (ctx.instance) {
      ...
      ctx.instance.emailVerified = true;
    }
    ...
    this.model.findById(loggedUser.userId, (err: any, user: any) => {
      if (err)
        ...
        user.updateAttribute(
            'loggedAt', new Date(), (err: any, user: any) => {});
    });
    next();
  }
  ...
  public afterRemoteCreate(ctx: any, userInstance: any, next: any) {
    ...
    this.model.count((err: any, countUser: any) => {
      if (err)
        console.error(err);
      else {
        if (countUser === 1) {
          ...
          this.model.app.models.Role.findOrCreate(
              ...
              userRole,                 // Create
              (err: any, instance: any, created: boolean) => {
                // Callback
                if (err)
                  console.error('Error creating role', err);
                else if (created) {
    ...
    if (process.env.MAILGUN_API_KEY && process.env.MAILGUN_DOMAIN &&
        process.env.MAILGUN_REGION && process.env.BASE_URL) {
      const verificationToken = generateVerificationToken();
      userInstance.updateAttributes({verificationToken});

      // Create a custom object your want to pass to the email template. You can
      // create as many key-value pairs as you want const verificationUrl =
      //   process.env.API_URL +
      ...
      const verificationUrl = process.env.API_URL +
          '/api/users/confirm?uid=' + userInstance.id +
      ...
      // Prepare a loopback template renderer
      const renderer = loopback.template(
          path.resolve(__dirname, '../../server/views/verify.ejs'));
      const html_body = renderer(customMessage);

      const options = {
        type: 'email',
        to: userInstance.email,
        from: 'Sigfox Connect <sigfox-connect@iotagency.sigfox.com>',
        subject: 'Welcome to Sigfox Connect!',
        html: html_body,
        redirect: '',
        user: userInstance,
      };
      ...
      => { if (err) { console.log(err); ctx.res.status(err.status || 500);
        }
        console.log('> verification email sent:', response);
      });*/
    }
  ...
  public beforeRemoteDelete(ctx: any, result: any, next: Function): void {
    ...
    User.findOne(
        {where: {id: userId}, include: 'Organizations'},
        (err: any, user: any) => {
          if (!err && user && user.Organizations) {
            user.toJSON().Organizations.forEach((orga: any) => {
              user.Organizations.remove(orga.id, (err: any, result: any) => {
                if (!err) {
                  console.log(
                      'Unlinked user from organization (' + orga.name + ')');
                }
              });
            });
          }
  ...
  public afterRemoteDelete(ctx: any, result: any, next: Function): void {
    ...
    RoleMapping.destroyAll(
        {principalId: userId}, (error: any, result: any) => {});
    ...
    // this.model.app.models.Dashboard.destroyAll({userId: userId}, (error: any,
    // result: any) => { }); this.model.app.models.Widget.destroyAll({userId:
    // userId}, (error: any, result: any) => { });
